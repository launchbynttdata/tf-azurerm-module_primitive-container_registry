...
.PHONY: go/lint
go/lint:
	find tests/ -name '*.go' | grep -q '\.go' || exit 0; \
	if [ "$(MISE_INSTALLED)" = "0" ]; then \
		mise exec go@$(GO_VERSION) golangci-lint@$(GOLANGCI_LINT_VERSION) -- \
			golangci-lint run -c .golangci.yaml --timeout 5m -v ./tests/...; \
	else \
		golangci-lint run -c .golangci.yaml --timeout 5m -v ./tests/...; \
	fi
